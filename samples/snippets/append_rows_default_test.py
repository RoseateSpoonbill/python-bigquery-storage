...
from . import append_rows_default
...
def test_append_rows_default(
    capsys: pytest.CaptureFixture,
    bigquery_client: bigquery.Client,
    sample_data_table: str,
):
    project_id, dataset_id, table_id = sample_data_table.split(".")
    append_rows_default.append_rows_default(
        project_id=project_id, dataset_id=dataset_id, table_id=table_id
    )
    out, _ = capsys.readouterr()
    assert "have been default" in out
    ...
    assert (("customer_name", "Luisa"), ("row_num", 7)) in row_items
    assert (("customer_name", "Alex"), ("row_num", 8)) in row_items
